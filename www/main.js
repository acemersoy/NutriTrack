...
class NutriTrack {
    ...
    // USDA API ile yiyecek arama (Cache destekli - localStorage + IndexedDB)
    async searchFoods(query) {
        ...
        // 2. IndexedDB cache'de ara (fallback)
        let cachedResults = [];
        try {
            if (typeof foodCache !== 'undefined') {
                cachedResults = await foodCache.searchFoods(query);
                if (cachedResults.length > 0) {
                    console.log(`📦 IndexedDB cache'den ${cachedResults.length} sonuç bulundu`);
                }
            }
        } catch (error) {
            console.warn('IndexedDB cache arama hatası:', error);
        }

        // 3. Cache'de yeterli sonuç varsa (5+), onları döndür (hızlı!)
        if (cachedResults.length >= 5) {
            console.log('⚡ IndexedDB cache\'den hızlı sonuç döndürülüyor');
        ...
        // 4. API'ye istek at (cache'de yoksa veya az sonuç varsa)
        ...
        try {
            ...
            console.log(`📊 API'den ${apiResults.length} sonuç alındı`);

            // 5. localStorage cache'e kaydet (24 saat TTL)
            ...
            // 6. IndexedDB cache'e kaydet (arka planda)
            if (apiResults.length > 0 && typeof foodCache !== 'undefined') {
                foodCache.saveFoods(apiResults).catch(error => {
                    console.warn('IndexedDB cache kayıt hatası:', error);
                });
            }

            // 7. Cache ve API sonuçlarını birleştir (duplicate kontrolü)
